...
interface EventStatisticsFormProps {
  event: TEvent;
}

export function EventStatisticsForm({ event }: EventStatisticsFormProps) {
  ...
  const totalRevenue = event.tarifications.reduce(
    (sum, ticket) =>
      sum + ticket.prix * (ticket.qteTotale - ticket.qteRestante),
    0
  );
  const potentialRevenue = event.tarifications.reduce(
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600">
          Analyse des performances pour "{event.description}"
        </p>
      </div>
